...
*/
void CloseSounds() {
	// 或者简单的关闭打开的所有媒体资源: mciSendString(_T("close all"), NULL, 0, NULL);
...
void PlaySounds(int kind) {
	...
	{
	case S_START:		
		mciSendString(_T("play start from 0"), NULL, 0, NULL);// play xxx from 0 表示从头开始播放; play xxx repeat 循环播放; pause xxx 暂停播放;		
		break;
	case S_SHOOT0:
